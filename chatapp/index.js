...
Chat.capitalize=function(name) {
	return name.replace(String(name[0]),String(name[0]).toUpperCase())
}
...
Chat.prototype.privateSay=function(message,user) {
	...
	for (var i=0; i < emoticons.length; i++) {
		...
		for (var j = 0; j < emoticons[i][0].length; j++)
			rgx += "["+emoticons[i][0][j]+"]";
		msg = msg.replace(new RegExp(rgx,"g"),'<img src="'+emoticons[i][1]+'" width="16" height="16" />');
		message = message.replace(new RegExp(rgx,"g"),'<img src="'+emoticons[i][1]+'" width="16" height="16" />');
	}
	...
	user.privateBlobInterval=setTimeout((function(user){return function() {
...
Chat.prototype.say=function(message,user) {
	...
	user.blobInterval=setTimeout((function(user){return function() {
...
Chat.prototype.changeRoom=function(index) {
	...
	if (Chat.roomNames[index].indexOf(".")>=0) {
		...
		this.embed.src = Chat.backgrounds[index].src+".gif";
	} else {
...
Chat.prototype.quit=function() {
	...
	hide($('avatarChatMain'));
	//On disconnection show something here
...
function loadLanguage(language) {
	...
	for (var i=0; i < rooms.length; i++)
		Chat.roomNames.push(rooms[i].firstChild.nodeValue+(rooms[i].getAttribute("animated")=="true"?".gif":""));
		...
		for (var i=0; i < children.length; i++)
			if (children[i].nodeName.toLowerCase()=="div") {
				...
				if (name.indexOf(".gif")>=0)
					name = name.substr(0,name.indexOf(".gif"));
...
window.onload=function() {
	...
	for (var i=0; i < avatars; i++) {
		...
		img.index = i;
		if (!(i%6) && i != 0) $("avatarChatAvatarImages").appendChild(document.createElement("br"));
